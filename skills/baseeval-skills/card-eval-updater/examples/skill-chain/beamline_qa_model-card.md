...
metrics:
    - beamline_qa/acc
    - beamline_qa/acc_norm
---
# lexical-overlap-baseline v1.0
...
> **Model identity supplied by the operator.** `lexical-overlap-baseline v1.0` was not recorded in the harness output and could not be verified against the run.

| Benchmark | Dataset | Items evaluated |
|---|---|---|
| beamline_qa | /path/to/card-eval-updater/examples/custom-benchmark/data/beamline_qa.jsonl | 24 of 24 |

**beamline_qa** — 24 four-way multiple-choice questions on synchrotron light-source science: accelerator physics, beamline optics, and X-ray techniques. Zero-shot. Random baseline 0.250; always-best-position baseline 0.333. Known artifact: correct answers are longer than distractors (38.8 vs 27.3 chars on average), so acc_norm is the honest headline metric and a high acc alone should not be read as domain knowledge.

*Generated by `card-eval-updater` from lm-evaluation-harness v0.4.10 output on 2026-07-30. Re-run the tool to refresh; do not edit this block by hand.*
...
- **Harness**: lm-evaluation-harness v0.4.10
- **Model evaluated**: `lexical-overlap-baseline v1.0`
- **Few-shot examples**: 0
- **Decoding**: harness defaults (not recorded in the run artifacts)
- **Compute**: cuda:0, batch size 1
- **Random seeds**: 0, 1234
- **Evaluation runtime**: 5s
- **Artifacts**: `examples/skill-chain/results/beamline_qa`
...
|---|---|---|---|---|---|
| beamline_qa | acc | 0.2083 | 0.0847 | — | — |
| beamline_qa | acc_norm | 0.2500 | 0.0903 | — | — |

Standard errors and standard deviations are reported by the harness; they are not recomputed here.
...
| beamline_qa | acc_norm | 0.2500 | 0.0903 | 24 |
